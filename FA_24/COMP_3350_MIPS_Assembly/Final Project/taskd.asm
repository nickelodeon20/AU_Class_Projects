...
###### MOVING COLOR CHANGE BACK INSIDE LOOP ######
...
# pointer to current pixel to start X loops; start at top-left corner
move $t3, $a2
...
# to inside loop
####################
li $t5, 20	 	 # Load value into unsused register
addu $t5, $t5, $t0	 # Add $t5 & $t0 to change color
addu $t0, $t5, $zero # color: changes in 24-bit RGB
#####################
...
#%TODO Task a: complete the following code
addu $a2,$a2,$t4	 # advace one row (left-edge pointer)
addu $t2,$t2,$t4	 # advace one row (right-edge pointer)
