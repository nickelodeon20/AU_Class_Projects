...
public class Doublets implements WordLadderGame {
   ...
   public List<String> getNeighbors(String word) {
      ...
      while (i < word.length()) {
         for (j = 0; j < 26; j++) {
            String newWord = createWord(word, i, j);
            if (isWord(newWord)) {
      ...
      for (j = 0; j < tempList.size(); j++) {
         // check if created words are Hamming Distance 1
         int num = getHammingDistance(word, tempList.get(j));
         if (num == 1) {
            neighbors.add(tempList.get(i));
         }
      }
   ...
   public boolean isWordLadder(List<String> sequence) {
      ...
      // check each word hamming distance is 1 from each other
   ...
   */
   ...
   // PSUEDO-CODE METHODS FROM NOTES
    ...
    private String createWord(String word, int WordI, int ArrayI) {
